...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        title = "Kids Stories App"

        val list: ListView = findViewById(R.id.list_view)
        val adapter = ArrayAdapter(this, android.R.layout.simple_list_item_1, titles)
        val settings: Button = findViewById(R.id.setting_btn)

        list.adapter = adapter
        list.setOnItemClickListener { _, _, position, _ ->

            val i = Intent(this, StoryActivity::class.java).putExtra("title", titles[position])
                .putExtra("story", stories[position]).putExtra("sound", sounds[position])
                .putExtra("picture", pictures[position])
            startActivity(i)
        }
